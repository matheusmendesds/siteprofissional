export default function Card({projeto}) {
    return (
        <a class="h-64 sm:h-80 lg:h-96 text-black">
            <div class="flex h-full transform items-end border-2 border-black bg-white transition-transform hover:-translate-x-2 hover:-translate-y-2">
                <div class="p-4 pt-0! transition-opacity  sm:p-6 lg:p-8">
                    <h2 class="mt-4 text-xl font-medium sm:text-2xl">{projeto}</h2>
                </div>

                {/* <div class="absolute p-4 opacity-0 transition-opacity hover:opacity-100 sm:p-6 lg:p-8">
                    <h3 class="mt-4 text-sm font-medium sm:text-2xl">{projeto}</h3>
